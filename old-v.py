...
GOAL_JUDGE_SYSTEM = """\
You are evaluating whether a goal has been achieved based on a Telegram conversation.
Be lenient — informal confirmations count as success regardless of who said them.
If anyone in the conversation confirms, agrees, or acknowledges the goal positively, mark it achieved.
A single "oui", "ok", "d'accord", or any positive signal counts.
Do NOT require the teacher specifically to confirm — any group member confirming is enough.
Reply ONLY with valid JSON:
{
  "achieved": true | false,
  "confidence": 0.0-1.0,
...
async def _dispatch(
    ...
    # Feed replies into goal queue if a loop is running
    ...
    #     # Still continue normal classification below
    ...
    if _goal_active.is_set() and combined:
        ...
        filter_resp = await loop.run_in_executor(None, lambda: llm.invoke([
            ...
            HumanMessage(content=(
                f"Active goal: {_pending_goal or '(goal in progress)'}\n\n"
    ...
    # Normal LangGraph classification
...
<script>
  ...
  function startCd(secs){
    ...
    cdTimer=setInterval(()=>{
      ...
      if(cdLeft<=0){clearInterval(cdTimer);doLogout();}
    },1000);
  }
...
#     global _pending_goal, _goal_status
#     goal = body.goal.strip()
#     _goal_status.update({"pending": False, "active": True, "last": "", "turn": 0})
#     loop = _event_loop
#     if loop:
#         asyncio.run_coroutine_threadsafe(_run_goal_loop(goal), loop)
#     log.info("Goal launched immediately via /task: %s", goal[:80])
#     return {"status": "launched", "goal": goal}

@app.post("/task")
async def set_task(body: TaskBody, _: bool = Depends(require_access)):
    global _pending_goal, _goal_status, GOAL_MAX_TURNS
    goal = body.goal.strip()
    GOAL_MAX_TURNS = body.max_turns          # ← apply the value
